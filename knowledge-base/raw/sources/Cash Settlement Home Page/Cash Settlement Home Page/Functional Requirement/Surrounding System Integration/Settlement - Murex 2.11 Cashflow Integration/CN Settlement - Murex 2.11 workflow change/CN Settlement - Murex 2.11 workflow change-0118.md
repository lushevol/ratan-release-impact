...
<xsl:stylesheet version="1.0" extension-element-prefixes="mx" xmlns:xsl="[http://www.w3.org/1999/XSL/Transform](http://www.w3.org/1999/XSL/Transform)" xmlns:mx="[http://murex.com/xslt/common](http://murex.com/xslt/common)">
    <xsl:template match="/">
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    <xsl:template match="/">
        <xsl:choose>
            <xsl:when test="MxPayMLResponse/MXG2000/flowID !=''">
                <xsl:value-of select="MxPayMLResponse/MXG2000/flowID"></xsl:value-of>
            </xsl:when>
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    <xsl:template match="/">
        <xsl:choose>
            <xsl:when test="MxPayMLResponse/sourceID !=''">
                <xsl:value-of select="MxPayMLResponse/sourceID"></xsl:value-of>
            </xsl:when>
...
<xsl:stylesheet version="1.0" extension-element-prefixes="mx"
  ...
  xmlns:mx="http://murex.com/xslt/common">
    <xsl:template match="/">
        <xsl:variable name="razorID">
      Mx#client.scb.fmrp.inbound.razorID#Mx
        </xsl:variable>
        <xsl:variable name="murexPayID">
      Mx#client.scb.fmrp.inbound.murexID#Mx
        </xsl:variable>
        <xsl:variable name="recordExistCheck">
                <xsl:value-of select="mx:execute-formula( 'client.scb.fmrp.inbound.recordCount', concat( 'FLOW_ID:', $murexPayID ))"></xsl:value-of>

        </xsl:variable>
        <xsl:choose>
            <xsl:when test="$razorID != '' and $murexPayID != 0 and $recordExistCheck > 0">
                <xsl:value-of select="normalize-space($murexPayID)"></xsl:value-of>
            </xsl:when>
            <xsl:otherwise>0</xsl:otherwise>
        </xsl:choose>
    </xsl:template>
...
```sql
select count(1) from SCB_FMRP_DBF where M_FLOW_ID=MxCTX#FLOW_ID#Mx
```
