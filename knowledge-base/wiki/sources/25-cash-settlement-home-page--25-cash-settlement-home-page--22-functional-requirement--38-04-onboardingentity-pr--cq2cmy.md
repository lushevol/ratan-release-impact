---
type: source
title: F2B Milestone Onboarding Checklist
authors: []
year: 2026
url: ""
...
created: 2026-08-22
updated: 2026-08-22
tags: [F2B, FMRP, cash-settlement, onboarding, auto-netting, functional-requirement]
related: [f2b, fmrp, murex, stella, ratan, entity-branch-onboarding, ssi-selection-hierarchy, nostro-static-management, auto-netting, netting-on-netting, cashflow-suppression, swift-mt-mx-integration, settlement-accounting, cash-settlement-migration, cdups, fmmis, tradehub, ebbs-rta, ccil, keystone, currency-transformation, duplicate-payment-prevention, go-live-decision-criteria]
...
# F2B Milestone Onboarding Checklist

## Summary

This source is an operational checklist for onboarding entities, branches, products, currencies, booking models, and settlement flows into the F2B/FMRP cash-settlement landscape.

It covers impacts across [[entities/fmrp]], [[entities/murex]], [[entities/stella]], [[entities/ratan]], SSI and Nostro static data, auto netting, cashflow suppression, SWIFT generation, accounting, migration, downstream routing, STP/NSTP controls, and go-live readiness.

The checklist is primarily a requirements-discovery and impact-assessment artifact. Many entries are questions, placeholders, or statements that functionality is not yet supported. It does not by itself demonstrate that a capability was approved, implemented, tested, or deployed.

## Scope and status caveats

- The source does not provide an authoring date or identify a specific onboarding milestone beyond the F2B milestone context.
- “Not supported yet” and “Supported Murex only” are status statements in the checklist and may require confirmation against current release evidence.
- Blank checklist rows do not establish that no change is required.
- The boundaries between Murex, STELLA, FMRP, and RATAN are not fully specified for field normalization, settlement-method stamping, or currency transformation.
- The golden source for Nostro static data is explicitly unresolved.
- Required go-live decision criteria and UVT verification points are named but not defined.

## Key findings

### Cross-system onboarding

Onboarding is presented as a coordinated change across static data, user interfaces, cashflow attributes, netting rules, SWIFT messages, accounting, migration, downstream routing, reporting, and operational controls. The affected systems and dependencies include [[entities/f2b]], [[entities/fmrp]], [[entities/murex]], [[entities/stella]], [[entities/ratan]], [[entities/ebbs]], [[entities/aspire]], [[entities/lms]], RAZOR, CDUPS, and FMMIS.

### Murex-to-STELLA/FMRP translation

Several rules and field mappings are described as Murex-specific. The checklist calls for explicit STELLA/FMRP equivalents for cashflow blotter fields, CCIL netting attributes, NDS auto netting, suppression filters, settlement-method stamping, strategy, typology, allocation, and message-field mappings.

### Currency normalization risk

The SGO/SGD example identifies a risk in which Murex produces SGD cashflows while Stella produces SGO cashflows. The checklist calls for currency transformation for SSI and Nostro lookup, while also marking the relevant capability as not yet supported in some paths. The ownership and implementation location of this normalization remain unresolved.

### Duplicate-payment prevention

Migration and swap-agent netting changes must prevent duplicate payments. The risk is specifically associated with package bookings, partial-entity migration, Murex write-back trades, already-paid Murex trades, and changes to swap-agent rules. The source does not define the authoritative payment key or deduplication algorithm.

### Constrained or unresolved capabilities

The checklist records the following constraints or open questions:

- Three-agent SSI structures are not supported.
- SSI and Nostro currency transformation is marked not supported yet.
- Bilateral auto netting is marked not supported yet.
- NDS auto netting is described as supported for Murex, with FMRP behavior unresolved.
- Netting over netting is restricted to IRS, with ND CCS identified as a requested configuration extension.
- IRS and Loan Depo interest netting across different trades is described as unsupported.
- The appropriate FMRP/STELLA design for CCIL settlement-method stamping is unresolved.
- The Nostro golden source is unresolved.
- Automatic DVP and possible eBBS RTA integration are open requirements.
- Go-live decision criteria and UVT verification points are not supplied.

## Exact structured source data

The original checklist table is preserved below, including terminology, capitalization, spelling, and field names.
...
- Where should SGO-to-SGD normalization occur: STELLA, FMRP, RATAN, or a lookup service?
- Which system is the golden source for Nostro settlement means and accounts?
- What is the approved Murex-to-STELLA/FMRP field mapping?
- How are duplicate payments prevented during package write-back and partial-entity migration?
- Is NDS auto netting supported in FMRP beyond Murex?
- Can ND CCS use netting over netting?
- Which FMRP events support undo and confirmation-match-driven settlement?
- What are the approved F2B go-live decision criteria and UVT verification points?

## Related wiki context

This source extends the existing pages for [[entities/f2b]], [[entities/fmrp]], [[entities/murex]], [[entities/stella]], [[entities/ratan]], [[concepts/entity-branch-onboarding]], [[concepts/ssi-selection-hierarchy]], [[concepts/nostro-static-management]], [[concepts/auto-netting]], [[concepts/netting-on-netting]], [[concepts/cashflow-suppression]], [[concepts/swift-mt-mx-integration]], [[concepts/settlement-accounting]], [[concepts/tag-20-logic]], and [[concepts/cash-settlement-migration]].

It should not be used alone to establish implementation status or production readiness.
